...
\usepackage{titlesec}
\usepackage{parskip}
\usepackage{grffile}   % suporte a espaços e acentos em nomes de arquivo
...
\author{
  Paulo Ricardo C. Rago \and
  Lucca Da Veiga \and
  Ronaldo Souto Maior \and
  Marina Frias \and
  Israel Erlich Neto
}
\date{Junho de 2026\\[0.4em]\small Grupo 8 — CESAR School}
...
um fator determinante para a estabilidade do sistema embarcado~\cite{esp32trm}.
...
mais uma atribuição final, totalizando $N$ atribuições~\cite{clrs,sedgewick}.
...
%─────────────────────────────────────────────────────────────────────────────
\subsection{Dashboard de monitoramento em tempo real}
%─────────────────────────────────────────────────────────────────────────────

Os dados do benchmark são transmitidos via MQTT pelo ESP32 e exibidos em tempo
real no dashboard web do ReciLuz (\texttt{/static/benchmark.html}).
As Figuras~\ref{fig:dash-grafico} e~\ref{fig:dash-tabela} apresentam capturas
de tela do dashboard durante uma execução real no ESP32-D0WD-V3 @ 240\,MHz.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Imagem 08-06-2026 às 14.55.jpeg}
  \caption{Dashboard ReciLuz — gráfico de latência por operação de inserção
    em escala linear. A curva vermelha (V1) cresce proporcionalmente a $N$;
    a linha verde (V2) permanece constante próxima de zero para todos os
    valores de $N$ testados.}
  \label{fig:dash-grafico}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Imagem 08-06-2026 às 14.55 (1).jpeg}
  \caption{Dashboard ReciLuz — histórico de benchmarks recebidos via MQTT.
    Cada linha corresponde a um valor de $N$ medido pelo ESP32 a cada 30
    segundos. A coluna \emph{Razão V1/V2} confirma o crescimento proporcional
    a $N$: de 294× em $N=100$ até 69.502× em $N=20.000$.}
  \label{fig:dash-tabela}
\end{figure}
...
implementado sobre FreeRTOS~\cite{freertos}, com tarefas em núcleos físicos
distintos do ESP32~\cite{esp32trm}:
...
via MQTT~\cite{mqtt,pubsubclient}.
...
\bibitem{esp32trm}
Espressif Systems,
\textit{ESP32 Technical Reference Manual}, versão 5.3.
Espressif Systems, Xangai, 2024.
Disponível em: \url{https://www.espressif.com/sites/default/files/documentation/esp32_technical_reference_manual_en.pdf}.

\bibitem{freertos}
R.~Barry,
\textit{Mastering the FreeRTOS Real Time Kernel — A Hands On Tutorial Guide}.
Real Time Engineers Ltd., 2016.
Disponível em: \url{https://www.freertos.org/Documentation/RTOS_book.html}.

\bibitem{mqtt}
OASIS Standard,
\textit{MQTT Version 5.0}.
OASIS, março de 2019.
Disponível em: \url{https://docs.oasis-open.org/mqtt/mqtt/v5.0/mqtt-v5.0.html}.

\bibitem{pubsubclient}
N.~O'Leary,
\textit{PubSubClient — MQTT library for Arduino}, versão 2.8.
GitHub, 2020.
Disponível em: \url{https://github.com/knolleary/pubsubclient}.
...
\bibitem{tanenbaum}
A.~S. Tanenbaum e H.~Bos,
\textit{Modern Operating Systems}, 4ª~ed.
